...
const { runZoneEngine } = require('./engine');

const DATA_DIR = path.join(__dirname, '..', '..', 'docs', 'data');
const REQUEST_DELAY_MS = 8000;
const OUTPUTSIZE = parseInt(process.env.ZONE_BACKTEST_OUTPUTSIZE || PARAMS.outputsize, 10);
const SYMBOL_FILTER = process.env.ZONE_BACKTEST_SYMBOL; // optional: run just one symbol (td or key)
...
async function main() {
  ...
  for (const sym of symbols) {
    ...
    for (const tf of TIMEFRAMES) {
      ...
      try {
        ...
        if (bars.length < 60) {
          ...
          const r = runZoneEngine(bars, PARAMS);
          results[tf.key] = {
            label: tf.label,
    ...
    fs.writeFileSync(
      path.join(DATA_DIR, `zone-backtest-${sym.key}.json`),
