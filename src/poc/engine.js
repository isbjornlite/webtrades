...
// Builds a volume profile across [zoneLow, zoneHigh] using bars[fromIdx..toIdx] inclusive.
// Uses real traded volume when present; falls back to counting bar presence (1 per bar
// touching a bin) when volume is missing or zero — the standard TPO-style substitute for
// instruments that don't report reliable volume (e.g. spot Gold/Silver).
function findPOC(bars, fromIdx, toIdx, zoneLow, zoneHigh, numBins) {
  const height = zoneHigh - zoneLow;
  if (height <= 0) return null;
  const binSize = height / numBins;
  const bins = new Array(numBins).fill(0);
  let usedRealVolume = false;

  for (let i = fromIdx; i <= toIdx; i++) {
    const b = bars[i];
    const weight = b.volume && b.volume > 0 ? b.volume : 1;
    if (b.volume && b.volume > 0) usedRealVolume = true;
    const lo = Math.max(b.low, zoneLow);
    const hi = Math.min(b.high, zoneHigh);
    if (hi < lo) continue;
    const binLo = Math.max(0, Math.floor((lo - zoneLow) / binSize));
    const binHi = Math.min(numBins - 1, Math.floor((hi - zoneLow) / binSize));
    for (let bin = binLo; bin <= binHi; bin++) bins[bin] += weight;
  }

  let bestBin = 0, bestWeight = -1;
  for (let bin = 0; bin < numBins; bin++) {
    if (bins[bin] > bestWeight) { bestWeight = bins[bin]; bestBin = bin; }
  }
  const pocPrice = zoneLow + (bestBin + 0.5) * binSize;
  return { price: pocPrice, usedRealVolume };
}

function runPocEngine(bars, PARAMS) {
  const n = bars.length;
  ...
  for (let f = 1; f < fractals.length; f++) {
    ...
    const fromIdx = Math.min(a.idx, b.idx);
    const toIdx = Math.max(a.idx, b.idx);
    const confirmedAt = toIdx + PARAMS.fractalStrength;
    if (confirmedAt < nextSearchIdx) continue;
    ...
    const poc = findPOC(bars, fromIdx, toIdx, zoneLow, zoneHigh, PARAMS.pocBins);
    if (!poc) continue;
    if (poc.usedRealVolume) anyRealVolume = true;

    // 1) impulsive breakout beyond the zone
    ...
    // 2) retest of the POC line specifically (not the whole zone)
    let retestIdx = -1;
    for (let i = breakoutIdx + 1; i < n; i++) {
      if (bars[i].low <= poc.price && bars[i].high >= poc.price) { retestIdx = i; break; }
    }
    ...
    if (exitPrice != null) {
      ...
      balance += pnl;
      closedTrades.push({
        direction, zoneHigh, zoneLow, poc: poc.price, entryTime: entryBar.time, entry, sl, tp, size, riskAmount,
  ...
  return { balance, closedTrades, usedRealVolume: anyRealVolume };
}

module.exports = { runPocEngine };
