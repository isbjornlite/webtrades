...
const { runOrbEngine } = require('./engine');

const DATA_DIR = path.join(__dirname, '..', '..', 'docs', 'data');
const REQUEST_DELAY_MS = 8000;
const LOOKBACK_DAYS = parseInt(process.env.ORB_LOOKBACK_DAYS || PARAMS.lookbackDays, 10);
...
async function main() {
  ...
  console.log(`ORB-backtest: ${LOOKBACK_DAYS} dager tilbake\n`);
  ...
  for (const sym of PARAMS.symbols) {
    ...
    const r = runOrbEngine(bars, PARAMS);
    results[sym.key] = {
      label: sym.label,
  ...
  fs.writeFileSync(
    path.join(DATA_DIR, 'orb-backtest.json'),
    JSON.stringify({ ranAt: new Date().toISOString(), lookbackDays: LOOKBACK_DAYS, params: PARAMS, results }, null, 2)
  );

  console.log('\nORB-backtest ferdig. Resultater lagret i docs/data/orb-backtest.json.');
}
