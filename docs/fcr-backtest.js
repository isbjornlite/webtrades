...
function renderParams(p) {
  return `<div class="card"><h2>Strategiparametere</h2>
    <div class="paramgrid">
      <div><span>FCR-vindu</span><span>${p.fcrStart}–${p.fcrEnd} EST</span></div>
      <div><span>Cutoff for oppsett</span><span>${p.cutoffTime} EST</span></div>
    ...
    <div class="footnote">Kun ett forsøk per dag. Hvis oppsettet (sveip + sveip + engulfing) ikke er fullført innen cutoff, hoppes dagen over.</div>
  </div>`;
}
...
async function render() {
  ...
  if (!cachedData) cachedData = await fetchJSON('data/fcr-backtest.json');
  const data = cachedData;

  if (!data) {
    status.textContent = 'Ingen backtest kjørt ennå.';
    main.innerHTML = `<div class="card"><div class="empty">
      Kjør workflowen "Backtest FCR opening-range strategy" fra Actions-fanen på GitHub for å generere resultater her.
    </div></div>`;
    return;
  }
