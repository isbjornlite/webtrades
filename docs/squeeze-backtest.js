...
function renderParams(p) {
  return `<div class="card"><h2>Strategiparametere</h2>
    <div class="paramgrid">
      ...
      <div><span>Bollinger</span><span>${p.bbPeriod}, ${p.bbMult}\u03c3</span></div>
      <div><span>Squeeze-terskel</span><span>bunn ${p.squeezePercentile}% av siste ${p.squeezeLookback} barer</span></div>
...
async function render() {
  ...
  if (!(activeSymbol in cache)) {
    cache[activeSymbol] = await fetchJSON(`data/squeeze-backtest-${activeSymbol}.json`);
  }
  ...
  if (!data) {
    ...
    main.innerHTML = `<div class="card"><div class="empty">
      Kjør workflowen "Backtest volatility squeeze strategy" fra Actions-fanen på GitHub for å generere resultater her.
    </div></div>`;
    return;
  }
