...
async function render() {
  ...
  if (!(activeSymbol in cache)) {
    cache[activeSymbol] = await fetchJSON(`data/zone-backtest-${activeSymbol}.json`);
  }
  ...
  if (!data) {
    ...
    main.innerHTML = `<div class="card"><div class="empty">
      Kjør workflowen "Backtest zone retest strategy" fra Actions-fanen på GitHub for å generere resultater her.
    </div></div>`;
    return;
  }
